function [K0,K,Omegaval]=LMI_TAC18_th2(A,B,C,Kbar,h,q,alpha,sigma)
...
% alpha    - decay rate 
% sigma    - event-triggering threshold from (13) 

% Output: 
% K0, K     - controller gains of (5), K0 is empty if not feasible 
% Omegaval  - event-triggering matrix from (13)
...
%% Decision variables 
P=sdpvar(n); 
Omega=sdpvar(m); % Phi<=0 implies Omega, W0, W{i}, R{i} >= 0
W0=sdpvar(m); 
W=cell(1,r-1); 
R=cell(1,r-1); 
for i=1:r-1
...
%% Notations 
M=kron(fliplr(vander([0 -q(:)'*h]))*(diag(1./factorial(0:r-1))),eye(l)); % the matrix from (8)
